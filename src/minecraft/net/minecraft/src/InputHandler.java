...
public class InputHandler {
		...
		for(int i0 = 0; i0 != gamepads.length; ++i0) {
			try {
				...
				if(!gamepads[i0].getName().toLowerCase().contains("controller (")) {
					...
					System.out.println("Captured: " + gamepads[i0].getName());
				}
	...
	public static void InputPOV(float f0) {
		if(f0 != lastPovX) {
			lastPovX = f0;
	...
	public static MobGiant SpawnGiant(double d0, double d2, double d4) {
		MobGiant mobGiant6 = new MobGiant(mc.theWorld, (int)mc.theWorld.milestone);
		mobGiant6.setLocationAndAngles(d0, d2, d4, mc.theWorld.rand.nextFloat() * 360.0F, 0.0F);
		mc.theWorld.spawnEntityInWorld(mobGiant6);
		return mobGiant6;
	}
	...
	public static boolean IsKeyDown(int i0) {
		boolean z1 = false;
		if(i0 == 57 && activeGamepad != null) {
			z1 = activeGamepad.isButtonPressed(0);
		}

		return Keyboard.isKeyDown(i0) || z1;
	}

	public static boolean InputMouseAction(int i0) {
		if(activeGamepad != null && !axis4isMinusOne) {
			boolean z10000;
			if(!Mouse.isButtonDown(i0)) {
				label37: {
					if(i0 == 0) {
						if(activeGamepad.getAxisValue(4) < -0.4F) {
							break label37;
						}
					} else if(i0 == 1 && activeGamepad.getAxisValue(4) > 0.4F) {
						break label37;
					}
