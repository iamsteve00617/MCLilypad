...
public class MapGenCaves extends MapGenBase {
	protected void generateLargeCaveNode(int i1, int i2, byte[] data, double d4, double d6, double d8) {
		this.generateCaveNode(i1, i2, data, d4, d6, d8, 1.0F + this.rand.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5D);
	}

	protected void generateCaveNode(int x, int z, byte[] data, double d4, double d6, double d8, float f10, float f11, float f12, int i13, int i14, double d15) {
		...
		for(boolean z26 = random23.nextInt(6) == 0; i13 < i14; ++i13) {
			...
			if(!z52 && i13 == i25 && f10 > 1.0F) {
				this.generateCaveNode(x, z, data, d4, d6, d8, random23.nextFloat() * 0.5F + 0.5F, f11 - (float)Math.PI / 2F, f12 / 3.0F, i13, i14, 1.0D);
				this.generateCaveNode(x, z, data, d4, d6, d8, random23.nextFloat() * 0.5F + 0.5F, f11 + (float)Math.PI / 2F, f12 / 3.0F, i13, i14, 1.0D);
				return;
			}
			...
			if(z52 || random23.nextInt(4) != 0) {
				...
				if(d4 >= d17 - 16.0D - d27 * 2.0D && d8 >= d19 - 16.0D - d27 * 2.0D && d4 <= d17 + 16.0D + d27 * 2.0D && d8 <= d19 + 16.0D + d27 * 2.0D) {
					int i53 = MathHelper.floor_double(d4 - d27) - x * 16 - 1;
					int i34 = MathHelper.floor_double(d4 + d27) - x * 16 + 1;
					int i54 = MathHelper.floor_double(d6 - d29) - 1;
					int i36 = MathHelper.floor_double(d6 + d29) + 1;
					int i55 = MathHelper.floor_double(d8 - d27) - z * 16 - 1;
					int i38 = MathHelper.floor_double(d8 + d27) - z * 16 + 1;
					if(i53 < 0) {
						i53 = 0;
					}
					...
					for(i40 = i53; !z56 && i40 < i34; ++i40) {
						for(int i41 = i55; !z56 && i41 < i38; ++i41) {
							for(int i42 = i36 + 1; !z56 && i42 >= i54 - 1; --i42) {
								...
								if(i42 >= 0 && i42 < 128) {
									if(data[i43] == Block.waterMoving.blockID || data[i43] == Block.waterStill.blockID) {
										z56 = true;
									}
					...
					if(!z56) {
						for(i40 = i53; i40 < i34; ++i40) {
							double d57 = ((double)(i40 + x * 16) + 0.5D - d4) / d27;

							for(i43 = i55; i43 < i38; ++i43) {
								double d44 = ((double)(i43 + z * 16) + 0.5D - d8) / d27;
								...
								for(int i48 = i36 - 1; i48 >= i54; --i48) {
									...
									if(d49 > -0.7D && d57 * d57 + d49 * d49 + d44 * d44 < 1.0D) {
										...
										if(b51 == Block.stone.blockID || b51 == Block.dirt.blockID || b51 == Block.grass.blockID) {
											if(i48 < 10) {
												data[i46] = (byte)Block.lavaMoving.blockID;
											} else {
												data[i46] = 0;
												if(z47 && data[i46 - 1] == Block.dirt.blockID) {
													data[i46 - 1] = (byte)Block.grass.blockID;
												}
											}
										}
									}
